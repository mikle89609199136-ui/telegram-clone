...
app.post('/api/register', (req, res) => {
    ...
    usersDB[email] = {id: userId, email: email.toLowerCase(), username: username.toLowerCase(),
        name: username.charAt(0).toUpperCase() + username.slice(1), avatar: '👤', password,
...
// ✅ MAIN HTML (ИСПРАВЛЕННЫЙ - без сломанных backticks)
app.get('/', (req, res) => {
...
<div class="auth-overlay" id="auth-overlay">
    <div class="auth-card">
        <div id="register-form">
            ...
            <p style="text-align:center;margin-top:20px;font-size:14px;"><a href="#" onclick="showLogin();return false;" style="color:#34c759;">Уже есть аккаунт?</a></p>
        </div>
        <div id="login-form" style="display:none;">
            <h2>Вход</h2>
            <input class="input-field" id="login-user" placeholder="Имя пользователя или Email">
            ...
            <p style="text-align:center;margin-top:20px;font-size:14px;"><a href="#" onclick="showRegister();return false;" style="color:#34c759;">Создать аккаунт</a></p>
        </div>
    </div>
</div>
...
<div id="chat-screen">
    <div class="chat-header">
        <button class="back-btn" onclick="backToList()" title="Назад">←</button>
        <div id="chat-title" style="font-weight:600;font-size:18px;">Чат</div>
    </div>
...
async function register(){
    ...
    let username=document.getElementById("reg-username").value.replace(/@/g,"").trim();
    let password=document.getElementById("reg-password").value;
    let confirm=document.getElementById("reg-confirm").value;
    if(!email||!username||!password){alert("Заполните все поля");return;}
    ...
    try{
        let res=await fetch("/api/register",{method:"POST",headers:{"Content-Type":"application/json"},
            body:JSON.stringify({email,username,password,confirmPassword:confirm})});
        let data=await res.json();
        if(data.success){alert("✅ Регистрация успешна! Войдите.");showLogin();
            document.getElementById("reg-email").value="";document.getElementById("reg-username").value="";
...
async function login(){
    ...
    try{
        ...
        if(data.success){
            ...
            socket.emit("join",currentUser.id);showApp();setTimeout(loadChats,100);
        }else{alert("❌ "+data.error);}
    }catch(e){alert("❌ Ошибка сервера");}
}
function showApp(){
    ...
    document.getElementById("chat-list").innerHTML="<div class='no-chats'>Загрузка...</div>";
}
async function loadChats(){
    try{
        let res=await fetch("/api/chats/"+currentUser.id);chats=await res.json();
    }catch(e){chats=[];}
    loadAllUsers();
}
async function loadAllUsers(){
    try{
        let res=await fetch("/api/users?exclude="+currentUser.id);allUsers=await res.json();
        renderChats();
    }catch(e){console.error(e);}
}
function renderChats(){
    let container=document.getElementById("chat-list");
    let hasChats=chats.length>0;
    let html="";
    if(!hasChats){
        allUsers.forEach(user=>{
            html+=`<div class='chat-item' onclick='openChat("${user.id}","${user.name}","${user.avatar}")'>
        ...
        chats.forEach(chat=>{
            html+=`<div class='chat-item' onclick='openChat("${chat.userId}","${chat.name}","${chat.avatar}")'>
...
async function searchUsers(){
    ...
    if(q.length<2){loadAllUsers();return;}
    try{
        let res=await fetch("/api/users?exclude="+currentUser.id+"&q="+encodeURIComponent(q));
        let users=await res.json();
        renderChats(); // показываем отфильтрованные
    }catch(e){}
}
async function openChat(userId,name,avatar){
    currentChat={id:userId,name:name,avatar:avatar};
    ...
    try{
        let res=await fetch("/api/messages/"+currentUser.id+"/"+userId);
        messages=await res.json();renderMessages();
...
function renderMessages(){
    ...
    messages.forEach(msg=>{
        let isSent=msg.from===currentUser.id;
...
function sendMessage(){
    ...
    socket.emit("message",{from:currentUser.id,to:currentChat.id,text:text});
    input.value="";checkSend();resizeInput();
}
socket.on("newMessage",data=>{
    if(currentChat&&(data.message.from===currentChat.id||data.message.to===currentChat.id)){
        messages.push(data.message);renderMessages();
    }
    loadChats();
});
...
if(savedUser){
    try{
        ...
        showApp();setTimeout(loadChats,500);
...
server.listen(PORT, () => {
    console.log('🚀 Zhuravlev Messenger V13.0 ✅ Запущен на порту ' + PORT);
});
